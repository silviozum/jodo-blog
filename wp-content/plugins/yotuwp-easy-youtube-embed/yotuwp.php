...
class YotuWP{
	...
	public function prepare( &$atts ){
		...
		switch ($atts['type']) {
			...
			case 'videos':
				$api_url = 'https://www.googleapis.com/youtube/v3/videos?part=id,snippet,contentDetails&maxResults='.$atts['per_page'].'&id='. $atts['id'];
				break;
	...
	public function load_more(){
		...
		$data = $this->prepare($atts);
		$atts_tmp = $atts;
		$atts_tmp['thumb_type'] = $thumb_type = $this->get_thumb_type($atts['column']);
	...
	public function template($template, $data, $settings){
		...
		if ( $overridden_template = locate_template( 'yotu'.YTDS. $template .'.php' ) ) {
			...
			load_template( $this->path . 'templates'.YTDS. $template . '.php', false );
		}
	...
	public function menu_page() {
		...
		add_submenu_page(
			'yotuwp',
			esc_html__('YotuWP', 'yotu'),
			esc_html__('General Settings', 'yotu'),
			'manage_options',
			'yotuwp',
			array($this->views, 'admin_page')
		);
	...
	public function media_button(){
		...
		$button_tip = __('Yotu - Embed Youtube', 'yotu');
		$icon = '<img src="'.$this->url . 'assets/images/yotu.png'.'"/>';

		echo '<a title="' . __('Yotu - Embed Youtube', 'yotu') . '" title="' . $button_tip . '" href="#TB_inline?width=700&height=550&inlineId=yotu_insert_popup" class="yotu-insert-btn button thickbox" >'.$icon.' YotuWP</a>';
	...
	public function get_thumb_type($column){
	...
	function admin_notice() {
		...
		echo '<div class="updated">
			<div class="yotu-notice-logo"></div>
				<h2>New feature : Shortcode Generator</h2>
				<p>I just created new feaure called <a href="admin.php?page=yotuwp-shortcode">Shortcode Generator</a>. That feature allow you create the YotuWP shortcode for using on widgets, product description, page builder or other place which support shortcode running. <a href="admin.php?page=yotuwp-shortcode">Check it now!</a></p>
				<ul class="yotu-rating-notice">
					<li>
						<span class="dashicons dashicons-email-alt"></span>
						<a href="https://www.yotuwp.com/contact" target="_blank">Contact Us</a>
					</li>
					<li>
						<span class="dashicons dashicons-dismiss"></span>
						<a href="admin.php?page=yotuwp&yotuwp_scgen_ignore_notice=yes">Dismiss</a>
					</li>
				</ul>
			</div>';
		}
	...
	public function lang_cfg(){
		$this->lang = array(
			'01' => __('Forbidden: You do not have permission to access resource on this server.', 'yotu'),
			'02' => __('Resource not found, please ensure you has correct information.', 'yotu'),
			'03' => __('Resource not found, please inform administrator about issues.', 'yotu'),
			'04' => __('Search Results:', 'yotu'),
			'05' => __('YotuWP: An issue happend when getting the videos, please check your connection and refresh page again.', 'yotu'),
			'06' => __('YotuWP: 500 Internal Server Error. Please inform administrator about issues.', 'yotu'),
		);
	}
}
